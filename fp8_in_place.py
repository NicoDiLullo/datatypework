...
in the e4m3FN FP8 format. Data is loaded (from float64), cast in place to the TensorFlow
e4m3FN FP8 format, and then to float 32, where the model is then trained as normal.
...
FP8_DTYPE = tf.experimental.float8_e4m3fn
################################################################################

def fp8_rdtrip(x):
    return tf.cast(tf.cast(x, FP8_DTYPE), tf.float32)
